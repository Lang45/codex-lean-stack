...
若本版改变提示决策，而且要声明正式安装后的新会话已经生效，则源码与静态规则通过后仍须：

- 先完成获授权的正式安装，再在全新项目目录创建独立父代理任务；
- 不向验收任务传入或让其读取源码项目 `Jiao-Jie.md`、原会话摘要、源码候选规则或人工策略
  摘录，只允许它使用自动加载的全局入口和正式安装缓存；
- 先以一个短确定性工具任务观察父代理直接处理且不委派，再以新的多工作流要求观察大块工作前
  的真实 `subAgentActivity` 或等价原生调用记录，并取得至少一个可采用结果；
- 单独验证模型路由的窄探针一旦出现预期启动事件和实际配置就停止；控制者只通知独立父任务，
