...
5.4.0 为多阶段工作加入低负担的状态呈现和恢复规则：只报告已证实状态、当前动作与下一项可观察
证据，恢复时只补缺失的最小权威状态；反复修补没有新增证据时转回根因或架构诊断，独立审查跨
多个提交时携带真实 `base..head` 范围。验证不再用 CHANGELOG 或交接中的同义子串冒充行为证据；
复杂流程优先走真实消费者的集成或端到端入口，全链审计保存重复归档比较和 CI 内复核过的
SHA-256 工件。解析、CAS、路径、权限、数据完整性和明确错误分支继续使用相称的隔离测试。
...
3. **停在首个完整方法。** 依次考虑不实现、复用现有实现、标准库、平台能力、官方支持的扩展点、既有依赖，以及安全、许可、兼容和升级成本可接受的成熟开源库；只有这些选项不能有效满足需求或总风险更高时，才增加最少自定义代码和文件。
...
4. **迭代测试链归调用功能。** 主任务精简确定检查对象、预期和受影响范围；`$lean-stack` 独立选择直接工具或有收益的验证子代理并收集结果，两者不形成技能加载或调用前置关系。新增行为或明确缺陷能先表达反馈时，先证明检查因预期行为缺口失败；复杂流程优先走可重复的集成或端到端入口并留下带版本与校验值的工件，隔离测试只用于更精确保护解析、CAS、路径、权限、数据完整性和明确错误分支。普通说明不靠跨 README、CHANGELOG 或交接的同义子串镜像维持。
