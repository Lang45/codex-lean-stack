...
1. 非平凡主任务先自动进入 `$lean-simplify`，由它选择最小完整步骤与最窄验证；只有选中的步骤需要模型或子代理时才进入 `$lean-stack`。两个功能仍分别验收、互不代证。
...
4. **迭代测试链归调用功能。** 主任务精简先确定检查对象、预期和受影响范围；`$lean-stack` 再选择直接工具或有收益的验证子代理并收集结果。
...
新建 Codex 会话后，直接使用这一条组合入口：

```text
先用 $lean-simplify 选择主任务必要步骤；只有步骤需要模型或子代理时再用 $lean-stack。
Use $lean-simplify to select the necessary main-task steps, then use $lean-stack only when a selected step needs a model or subagent.
```
