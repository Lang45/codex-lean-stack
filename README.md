...
1. 一个插件内提供两条并行、分别验收的功能：工具与 PowerShell 路径精简由两边共同遵守；`$lean-simplify` 负责主任务精简，只有步骤需要模型或子代理时才由 `$lean-stack` 决定调用路线。两者互不代证。
...
4. **调查与实现只读需要的手册。** 测试、复核和重验只覆盖真实受影响范围；条件性语义复核与最终验证、失败后的最窄重验分别按触发条件进入。
...
2. **子代理先公开实际配置与保留状态。** 第一条可见回复以本地化名称、模型、思考程度和速度
...
使用 $lean-simplify 做主任务精简。
Use $lean-simplify to simplify the main task.
```
