...
5.3.3 把两个入口的条件表写成“当前状态 + 下一项实际动作”，不要求用户说出“调查”“审查”等
条件名称；判断说明紧邻路由表，命中后才读取对应小分支。同一任务仍可依次读取多个命中分支，
不批量预读完整 reference，也不因拆分丢失后继调用、来源、写入、汇合或保留触发。持久化身份
统一称为“保留子代理”，并继续按可复用能力、权限和证据形状匹配。源码更新、安装成功或缓存
一致仍不能证明已运行会话热加载了新规则。
