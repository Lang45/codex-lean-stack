...
1. 一个插件内提供两条并行、分别验收的功能：`$lean-simplify` 负责主任务精简；步骤涉及工具、模型或子代理时，同时由 `$lean-stack` 决定调用路线。两者互不代证。
...
1. **子代理调用 `$lean-stack`。** 负责工具和 PowerShell 路径精简，调用判断与模型、成本、时间选配，上下文、来源和结果复用，内部交流与子代理结果收口，角色、经验和生命周期表面精简，以及迭代测试链。
2. **主任务精简 `$lean-simplify`。** 负责最小完整任务方法，主任务状态与沟通收窄，调查、实现和任务手册，测试、复核和重验范围，反过度工程与维护面，显式消融，发布、安装和文档维护，以及条件性语义复核与最终验证链、失败后的最窄重验链。
3. **互不代证。** 主任务精简不要求先调用子代理；复杂任务可同时使用两个入口，但调用记录不能证明主任务已精简，静态精简规则也不能证明子代理运行成功。
4. **角色经验归调用功能。** `agents.py` 的角色、运行、经验与摘要维护是子代理生命周期，不是源码、设计或规则精简，也不会自行启动子代理。
...
5. **上下文按需。** 独立任务优先 `fork_turns="none"`；只传当前约束和证据。
