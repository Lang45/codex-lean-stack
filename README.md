...
1. 路由顺序固定为质量、成本、时间；资源成本不再混入墙钟时长，处于可接受成本带的少量低成本并行可以换取更短关键路径。父代理用 `MODEL_ROUTE` 留下可审计的四模型选择依据。
2. `gpt-6-astra` 只处理 GPT-5.6 存在决定性质量差距或能降低整项资源成本的当前专家切片，普通视觉任务不触发，也不要求低价模型先失败；派出的 Astra 最高为 `xhigh`。子代理第一条可见回复先给本地化名称、模型、思考程度和速度四行，前面不出现计划或运行 ID，且不回显 `run_id`。
3. 完整保留角色 TOML 不设置 KiB 总大小上限；单项角色说明、活动记忆、迁移计划、受管文件数量等逐项边界以及真实 TOML 往返校验继续保护功能和数据。规则只保留一个权威源，并要求真实消费者、触发条件和可观察证据，避免规则越写越多却仍不可执行。
...
2. **子代理先公开实际配置。** 第一条可见回复以本地化名称、模型、思考程度和速度四行开头，前面不出现计划或运行 ID；普通任务不再向父代理重复发送相同四行。
