...
5. 插件不规定项目交接文件的创建、读取、更新或结构；这些由用户和项目或全局规则决定。插件只识别 `lean-simplify` 与 `lean-stack` 两个按需入口，当前动作命中哪个才读哪个，不能在开头通读两份。
6. 同一长来源由一个所有者完成发现与完整读取；其他子代理复用 `SOURCE_ROUTE` 证据包，只定向补
   具名缺口或必要的高风险事实。
7. 保留子代理开场在“专家角色、模型、思考程度”三行配置后显示存活轮次和经验状态，仍不显示父侧 `run_id`；经验版本可以
...
4. **新会话直接开始。** 识别 `lean-simplify` 与 `lean-stack` 两个按需入口，当前动作命中哪个才读哪个，不能在开头通读两份或增加第二段初始化前言。无关记忆、完整细则、委派或版本解释不能阻塞主任务，也不能预造用户未要求的交付物。
5. **四模型联合选配。** 不设调用数量；父代理按任务类型与任务类型组复用保留子代理或定制新子代理，一次联合选择模型与思考程度，并保留 `MODEL_ROUTE` 收据。Luna 处理清楚易验切片，Terra 处理有限语义歧义，Sol 承担复杂端到端工作；Astra 仅处理 GPT-5.6 仍有决定性差距或能降低整项资源成本的当前专家问题，最高 `xhigh`，普通视觉任务不触发。`spawn_agent` 没有每次委派的速度、`service_tier` 或 `priority` 参数；具名自定义角色可预配置 `service_tier`，但它不是本次选配轴，也不进入任务卡或可见声明；通用子代理继承会话全局 Fast mode。
...
2. **子代理先公开实际配置与保留状态。** 父代理把“专家角色、模型、思考程度”三行实际配置及
   “存活轮次、经验状态”两行保留状态写进每个直接派发任务卡，不假定通用 `explorer`、`worker` 或继承历史会自动取得保留角色提示。第一条可见回复依此顺序开头；保留角色采用 `recall` 的客观状态，运行时角色说明 0 轮和未加载保留经验，不要求子代理声明经验适用性。五行前不出现计划或运行 ID；最终只重复三行配置。
