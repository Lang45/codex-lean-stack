...
5. **三原则决定调用。** 高价值工作先守必要质量和决定性证据；质量达标后先比较父代理继续读取、实现或诊断所需的 token、时间和返工，与子代理启动、交流、核验、整合及返工组成的候选路线总成本，不能只把启动子代理算作新增成本，成本相近才比较总完成时间。当前用户明确速度或期限要求时再提高速度权重；没有必要质量收益时，不为单纯提速大幅增费。正向寻找能替代父代理阅读、实现或分析的切片，端到端任务也可委派。
...
8. **联合选择完整配置。** 所有父代理按三原则比较完整的“模型 + 思考程度 + 速度”组合，三个字段可以一起调整，同一模型不固定一档；Luna、Terra、Sol 是通常候选，Sol 可端到端承担复杂高价值工作。Astra 只在额外决定性质量或更少返工、token 等使全任务总成本更低时考虑，不能只凭更快。Sol 为父代理时，主要判断是遇到自身难以可靠解决的具体困难，而且确实需要专家协助；满足时可以把该未决问题作为有界 Astra 子任务。UI 设计、图片、三维建模、渲染及其他视觉工作需要专家帮助时也适用；最终评审或验收核对只是辅助场景，不是前置门槛。视觉任务本身不触发。不要求先制造一次低价模型失败，也不升级整批。新选配全部默认标准速度；只有当前用户明确速度或期限要求且完整路线有收益时才用快速。已有显式快速配置保留真实值，不适配时只调整本次调用。
...
5. **持续多工作流任务尽早派发并重判。** 多个独立就绪切片能替代父代理实际研究、实现或验收时，在父代理深入读取额外来源族或开始实现前，派发全部当前有收益且互不冲突的 GPT-5.6 切片；数量随真实工作流、边际收益与容量变化，不设最低数量或逐片量化门槛，无法精确量化本身不能否决调用。原本直接处理的判断只对当前任务形状有效；新需求跨越系统或证据范围、具体困难暴露独立诊断切片，或压缩后出现多个就绪工作流时，先重新做正向派发判断再继续大块父代理工作。每个新增同类型切片仍按父代理继续工作的成本也在内的全任务总成本、必要质量收益，以及成本相近且用户有当前期限要求时的总完成时间判断。
...
10. **委派替代重复劳动。** 工具先定位并批量读取独立来源，在工具侧筛选和限制输出预算，再在昂贵父代理大量预读或生成前交给合适角色；已落盘结果只回定位、关键差异与验证。父代理保留用户互动、共享热点、整合和必要验收，不全面重做；父代理继续工作的 token、时间和返工也计入候选路线，按父子合计消耗、交流与返工评估成本。
11. **依赖证据才直达队友。** 只有真正解锁下一动作的来源和发现才发内部消息；影响职责、共享写入或安全的变化同时告知父代理裁决，不等待确认，也不代交队友最终结果。

### 收口与复用

1. **每个子代理独立交付。** 子代理达到成功条件或停止条件后，必须在自己的线程提交明确的最终回复或可采用收据；只有 commentary 中间消息不算交付。普通子代理不能代交、隐藏或汇总其他子代理的最终回复；若只剩具体缺口，父代理至多做一次有界补问，否则关闭或改路，不全面重做。
...
8. **Select one complete configuration.** Every parent compares complete model, reasoning, and speed combinations under the three principles. Luna, Terra, and Sol are the usual candidates; Sol can own complex high-value work end to end. Use Astra only for additional decisive quality or a lower whole-task cost through less rework or fewer tokens, not merely because it is faster. When Sol is the parent, the primary condition is a concrete problem that Sol cannot resolve reliably and that genuinely needs expert assistance; Sol may give that bounded problem to Astra. The same applies when UI design, images, 3D modeling, rendering, or other visual work needs expert help. Final review or acceptance is an auxiliary use case, not a prerequisite, and a visual task by itself does not trigger escalation. This does not require manufacturing a lower-cost model failure and does not upgrade the whole batch. New selections default to Standard speed, including Luna. Use Fast only for an explicit current deadline or speed request when the complete route benefits; preserve an existing explicit Fast setting and adapt only the current call when it no longer fits.
...
5. **Dispatch sustained parallel workflows early and rejudge.** When multiple independent ready slices can replace the parent's research, implementation, or acceptance work, dispatch all currently beneficial, non-conflicting GPT-5.6 slices before the parent deep-reads additional source families or begins implementation. The count follows actual workflows, marginal benefit, and runtime capacity; there is no minimum count or per-slice quantified-savings threshold, and inability to quantify precisely is not a veto. A direct-handling decision applies only to the current task shape; when a new request crosses system or evidence boundaries, a concrete difficulty exposes an independent diagnostic slice, or compression leaves multiple ready workflows, rejudge positive dispatch before more substantial parent work. Each additional same-type slice still follows whole-task cost including continued parent work, required quality, and, when costs are close and the user has a current deadline, completion time.
...
10. **Source coverage is explicit.** Batch independent reads, filter and budget output in tools, and return `SOURCE_COVERAGE` with the snapshot, coverage level, and remaining gap instead of flooding the parent with raw logs. Preserve the parent's user interaction, shared hotspots, integration, and necessary acceptance; continued parent tokens, time, and rework remain part of the candidate route cost.
...
1. **Each agent submits its own result.** A subagent must submit an explicit final response or adoption receipt after meeting its success or stop condition; commentary-only progress is not a deliverable. Ordinary agents cannot hide, replace, or submit another agent's final response. If one concrete gap remains, use at most one bounded follow-up; otherwise close or reroute instead of redoing the whole task.
