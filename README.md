...
4. **新会话直接开始。** 在同一次启动热路径中快速实际读取已加载的 `lean-simplify` 与 `lean-stack` 两个按需入口，不预读它们链接的参考细则，并立即执行第一个实际动作；两入口随后独立或并行触发，入口未变化时后续轮次直接复用。无关记忆、保留目录、委派或版本解释不能阻塞主任务，也不能预造用户未要求的交付物。
